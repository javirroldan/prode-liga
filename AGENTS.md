...
- **Auto-recuperación de sesión en PWA**: La sesión vive solo en cookies `sb-*` (400 días); algunos Androids/browsers OEM las limpian al cerrar la app instalada y pedían login siempre. Solución (08/2026): (1) `client.ts` espeja access/refresh token en localStorage (`prode-liga-auth-backup`) vía onAuthStateChange — el listener se adjunta en TODAS las páginas gracias a `<AuthBackup />` montado en el root layout (sin eso el dashboard/admin nunca instanciaba el browser client y el backup no se escribía); (2) server action `syncSession` restaura cookies desde ese backup con `setSession` (valida contra Auth server); (3) helper compartido `src/lib/recover-session.ts` (`recoverSession()`: getSession → si no hay, syncSession con backup) corrido al montar por la login page Y por `<SessionRecovery />` en el landing `/` — así la app abre directo en dashboard tengas cookies o no (el landing además hace redirect server-side con `getCurrentUser()` si hay cookies vivas); (4) el logout manual en `navbar.tsx` limpia el backup ANTES de llamar a la server action `logout` (si no, la recuperación re-restauraría la sesión); sw.js nunca cachea `/auth/*` ni respuestas redirigidas. El flag `?recovery=1` del middleware quedó como señal pero NO es requerido. Si se toca auth, no romper este flujo
...
- **PWA**: Instalable y con soporte offline (ver PWA-GUIDE.md para la guía de referencia). Piezas: `src/app/manifest.ts` (servido en `/manifest.webmanifest`), `public/sw.js` (precache mínimo + network-first para HTML + cache-first/runtime caching para estáticos; ignora `/api/*` y requests cross-origin como Supabase; SOLO intercepta GETs — `req.method !== "GET"` hace return temprano para que los POSTs/Server Actions como guardar pronósticos o resultados pasen nativos sin pasar por caché) y registro inline en `layout.tsx` (solo production). El middleware NO debe interceptar `/sw.js` ni `/manifest.webmanifest` (excluidos en el matcher de `src/middleware.ts`)
- **Íconos PWA**: deben ser PNG **cuadrados exactos** o Chrome deja de ofrecer "Instalar app": `icon-192x192.png` 192×192, `icon-512x512.png` 512×512, `apple-touch-icon.png` 180×180. Regenerados (08/2026) desde el nuevo logo de la app (`iconlogo.jpg`, fuente cuadrada 1254×1254 en la raíz del repo) con `magick -filter Lanczos -resize NxN`. Si se cambia el ícono: regenerar los 3 PNG desde la fuente respetando dimensiones exactas
